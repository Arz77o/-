...
const CORS_HEADERS = {
  ...
  "Access-Control-Allow-Headers": "authorization, content-type, apikey, x-client-info, x-purchase-secret",
...
serve(async (req) => {
  ...
  if (body.event_name === "Purchase") {
    ...
    if (purchaseSecret !== PURCHASE_SECRET) {
      return new Response(JSON.stringify({ error: "Unauthorized: invalid purchase secret" }), {
        status: 401,
        headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
      });
    }
